...
env = MM2DRobotCGymEnv(maxstep=MAX_EP_STEPS, space=1, ifobs=1)
rec = VideoRecorder(env, path=parentdir+'/neobotix_schunk_pybullet/results/ddpgrollout1.mp4')
...
with tf.name_scope('Return'):
    Return = tf.placeholder(tf.float32, name='return')
...
tf.summary.scalar('epi_returns', Return)
...
def train():
    ...
    for ep in range(MAX_EPISODES):
        ...
        for t in range(MAX_EP_STEPS):
            ...
            Nep += 1
            #sess.run(critic.loss, feed_dict={S: s.reshape(1,STATE_DIM), critic.a: a.reshape(1,ACTION_DIM), R: [[r]], S_: s_.reshape(1, STATE_DIM)})
            s = s_
            ep_step += 1
            returns += r
        ...
        if True:
            summary = sess.run(merged, feed_dict={Return: returns})
