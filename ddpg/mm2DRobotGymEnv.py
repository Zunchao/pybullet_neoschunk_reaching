...
class MMRobotGymEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.pee = self._observation[-2:]
        self.plinks = self.transform_joint_position(self.robot_state[5:8], self.linklength, self.robot_state[0:2],
                                                    self.robot_state[4])
        if self.has_obs:
            while True:
                self.pobs[0] = np.random.uniform(0, self.pee[0])
                self.pobs[1] = np.random.uniform(0, self.pee[1])
                if np.linalg.norm(self.pobs)>0.2 and not self.check_collison(self.plinks):
                    break
    ...
    def get_observation(self):
        ...
        observation[8:10] = self.plinks[4:6]
        if self.has_obs:
    ...
    def terminator(self):
        ...
        if self.has_obs:
            if self.check_collison(self.plinks):
    ...
    def check_collison(self, p):
        for i in [0, 2, 4]:
            #print(self.pobs,p[i:i+2])
            dis = np.linalg.norm(self.pobs-p[i:i+2])
            if dis < 0.15:
                return True
        return False
...
if __name__ == '__main__':
    # np.random.seed(11)
    env = MMRobotGymEnv(maxstep=2e2, space=1, ifobs=True)
