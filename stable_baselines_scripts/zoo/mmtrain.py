...
python stable_baselines_scripts/zoo/mmtrain.py --algo ppo2 --env NeobotixSchunkBulletEnv-v0 --n-timesteps 10000000 --log-folder logs/0923prio --tensorboard-log ~/results_server/stable-baselines/0923prio -optimize --n-jobs 6 --sampler random --pruner median
without tuning:
python stable_baselines_scripts/zoo/mmtrain.py --algo ppo2 --env NeobotixSchunkBulletEnv-v0 --n-timesteps 10000001 --save-freq 1000000 --log-folder logs/0923prio --tensorboard-log ~/results_server/stable-baselines/0923prio
'''
import os
import time
import inspect
...
#ENVIRONMENT = NeobotixSchunkGymEnv(renders=False, isDiscrete=False, maxSteps=2048, action_dim=9, colliObj=0, wsboundary=1, randomInitial=0)
if __name__ == '__main__':
    ...
    with open(currentdir+'/hyperparams/{}.yml'.format(args.algo), 'r') as f:
    ...
    def create_env(n_envs, eval_env=False):
        ...
        if is_atari:
            ...
            if n_envs == 1:
                ...
                # env = DummyVecEnv([make_env(env_id, i, args.seed, log_dir=log_dir, wrapper_class=env_wrapper, env_kwargs=env_kwargs) for i in range(n_envs)])
            if normalize:
                if args.verbose > 0:
