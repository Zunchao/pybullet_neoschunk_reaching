...
if __name__ == '__main__':
    ...
    data4 = pd.read_csv('/home/zheng/resultsss/results/success_rate_roha_steps0915rohaprio.csv', delimiter=',',
                        names=['Episode', 'Steps'])
    ...
    '''
    fig, ax = plt.subplots()
    sns.relplot(x='Episode', y='Success Rate', kind="line", data=data1, color="b", ax=ax, lw=5, estimator="max", ci=None)
    sns.relplot(x='Episode', y='Success Rate', kind="line", data=data2, color="r", ax=ax, lw=5, estimator="max", ci=None)
    #sns.relplot(x='Episode', y='Success Rate', kind="line", data=data3, color="g", ax=ax, lw=5)
    ...
    plotlegend = ['Base reward', 'Prioritized reward']
