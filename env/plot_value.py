...
if __name__ == '__main__':
    ...
    data1 = pd.read_csv(
        '/home/zheng/resultss/logs/noprio1/ppo2/NeobotixSchunkBulletEnv-v0_1/monitor_all.csv',
        delimiter=',')

    data2 = pd.read_csv(
        '/home/zheng/resultss/logs/prio1/ppo2/NeobotixSchunkBulletEnv-v0_2/monitor_all.csv',
        delimiter=',')
    '''data3 = pd.read_csv('/home/zheng/results_server/results/success_rate_withobs28d_moving.csv', delimiter=',',
                        names=['Episode', 'SuccessTotal', 'Success Rate', 'SuccessUpdate', 'SuccessRateUpdate'])
    ...
    '''
    fig, ax = plt.subplots()
    sns.relplot(x='t', y='r', kind="line", data=data1.round(), color="b", ax=ax, lw=1)
    sns.relplot(x='t', y='r', kind="line", data=data2.round(), color="r", ax=ax, lw=1)
    ...
    plotlegend = ['static obstacle env', 'static obstacle env \nwith transfer learning']
    #plotlegend = ['no obstacle env']
    ax.legend(labels=plotlegend, loc='lower right', numpoints=1, fontsize=50)
    ax.tick_params(labelsize=50)
    #ax.set_xlim(xmax=200000000)
    #ax.set_ylim(ymax=0)
    ax.set_xlabel('Step', fontsize=50)
    ax.set_ylabel('Return', fontsize=50)
    # ax.set_xlabel('xlabel', fontsize=10)
    plt.show()