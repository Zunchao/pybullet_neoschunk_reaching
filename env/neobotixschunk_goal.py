...
class NeobotixSchunkGoal:
    def __init__(
        ...
        self.goal_uid = self.pb.createMultiBody(
            baseMass=0, baseVisualShapeIndex=goal_id, basePosition=self.goal_position
        )
        self.goal_orientation = np.array([0, 0, 0, 1])
        self.resetGoal()
    ...
    def resetGoal(self):
        ...
        self.pb.resetBasePositionAndOrientation(
            self.goal_uid, self.goal_position, np.array([0, 0, 0, 1])
        )
        self.goal_circle_r = np.sqrt(
    ...
    def resetGoalScenarioRandom(self):
        ...
        self.goal_position = np.array(
            [
                self.np_random.uniform(-self.ws_boundary, self.ws_boundary),
                self.np_random.uniform(1.5, 2.5),
                self.np_random.uniform(GOAL_HEIGHT_LOW, GOAL_HEIGHT_HIGH),
            ]
        )
        self.pb.resetBasePositionAndOrientation(
    ...
    def getGoalState(self):
        ...
        :return:
        """
        basev = self.pb.getBaseVelocity(self.goal_uid)
        pos, orn = self.pb.getBasePositionAndOrientation(self.goal_uid)
        ...
        self.pb.resetBasePositionAndOrientation(self.goal_uid, self.goal_position, orn)
    ...
    def setGoalStateRandom(self):
        ...
        if self.if_goal_moving_type == "random":
            self.goal_linear_velocity += np.array(
                [
                    self.np_random.uniform(-GOAL_VEL, GOAL_VEL),
                    self.np_random.uniform(-GOAL_VEL, GOAL_VEL),
                    self.np_random.uniform(-GOAL_VEL, GOAL_VEL),
                ]
            )
            self.goal_linear_velocity = np.clip(
                self.goal_linear_velocity,
                -np.array(GOAL_VEL_LIMITS),
                np.array(GOAL_VEL_LIMITS),
            )
        self.pb.resetBaseVelocity(
            objectUniqueId=self.goal_uid,
    ...
    def setGoalStateInCircle(self):
        ...
        """
        pos, orn = self.pb.getBasePositionAndOrientation(self.goal_uid)
        arm_base_ang = self.pb.getEulerFromQuaternion(orn)
