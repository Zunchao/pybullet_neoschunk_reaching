...
class NeobotixSchunkGymEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.goal = neobotixschunk_goal.NeobotixSchunkGoal(urdf_root_path=self.urdf_root, ws_boundary=self.ws_boundary, rseed=self.np_random, if_goal_moving_type=self.if_goal_moving_type)
        self.goal.resetGoal()
    ...
    def check_collision_obs(self):
        ...
        closest_distances = []
        if self.if_obstacle:
            closest_points = p.getClosestPoints(self.robot.neobotix_schunk_uid, self.obstacle.obstacle_uid, COLLISION_THRESHOLD)
    ...
    def _termination(self):
        ...
        if self.check_collision_obs():
            ...
            if self.dis_collision > COLLISION_THRESHOLD:
                ...
                self.r_penalty_collision = np.log(self.dis_collision)
            else:
                self.r_penalty_collision = -5
                self.r_termination = -100
                self.terminated = 2
    ...
    def _reward(self):
        ...
        if self.reward_type == 'rdense':
            ...
            reward = 100*ree + self.r_termination + 50*self.r_penalty_collision - 40*self.u**2 - 10*(self.step_counter_per_episode/self.max_steps)**2
            if self.with_prioritized_reward:
                reward = 100 * rp + self.r_termination + 50 * self.r_penalty_collision - 40 * self.u ** 2 - 10 * (self.step_counter_per_episode / self.max_steps) ** 2
            if self.if_goal_moving_type == 'line':
    ...
    def render(self, mode='rgb_array', close=False):
        if mode != "rgb_array":
            return np.array([])
        base_pos, orn = p.getBasePositionAndOrientation(self.robot.neobotix_schunk_uid)
        ...
        p.addUserDebugLine(self.former_ee_pos, self.ee_position, [0, 0, 1], 3)
        p.addUserDebugLine(self.former_base_pos, self.base_position, [0, 1, 0], 3)
        if self.if_goal_moving_type is not 'static':
            p.addUserDebugLine(self.former_goal_pos, self.goal.goal_position, [1, 0, 0], 3)
