...
class NeobotixSchunkObstacle:
    def __init__(
        self, urdf_root_path=None, ws_boundary=1, rseed=None, if_obstacle_moving=False
    ):
        ...
        self.URDF_OBSTACLE = os.path.join(
            self.urdf_root, "pybullet_neoschunk_reaching/data/cylinder_verticle.urdf"
        )  # unused
        obstacle_id = self.pb.createCollisionShape(
    ...
    def getObstacleState(self):
        ...
        """
        if self.if_obstacle_moving:
            obsvel = self.pb.getBaseVelocity(self.obstacle_uid)
            self.obstacle_linear_velocity = np.array(obsvel[0])
            obspos, obsorn = self.pb.getBasePositionAndOrientation(self.obstacle_uid)
